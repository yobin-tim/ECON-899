...
module params_grid
    ...
    DOUBLE PRECISION            :: THETA                      ! Labor income tax rate
...
program conesa_krueger
    ...
    ! Read User supplied values for THETA
    CALL GET_COMMAND_ARGUMENT(1,THETA_char)
    READ(THETA_char,*)THETA
...
subroutine coda()
    ...
    open(unit=2, file='./PS4/FortranCode/results.csv', status='replace', action='write', iostat=rc1)
...
write(2,200)j,grid_Z(i_Z),grid_A(i_A),pf_l(i_A, i_Z, j),pf_c(i_A, i_Z, j),pf_A(i_A, i_Z, j),pf_A_ind(i_A, i_Z, j),pf_v(i_A, i_Z, j)
    ...
    open(unit=3, file='./PS4/FortranCode/agg_results.csv', status='replace', action='write', iostat=rc2)
